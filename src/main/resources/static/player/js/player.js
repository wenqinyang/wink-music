...
var lzxPlayerInit = function () {
    ...
    const jsUrl = $("#wink").attr("src");
    var webURL = jsUrl.startsWith("http") ? jsUrl.substring(0,jsUrl.indexOf("/",8)) : window.location.origin;
    var keyId = isTest ? testKey : $("#wink").attr("key");

    if(!styleLoaded){
        $("head").append('<link rel="stylesheet" type="text/css" href="' + webURL + '/music/api/resource/css/player.css">');
    ...
    $songFrom.html('<a style="color:#f00">www.winke.com</a>');
    ...
    var lzxMedia = {
        ...
        error: function () {
            clearInterval(cicleTime);
            $player.removeClass('playing');
            lzxTips.show(responseData.songSheetList[albumId].songNames[songId] + ' - 资源获取失败！尝试获取下一首...');
            lzxMedia.next();
        },
        seeking: function () {
            lzxTips.show('加载中...')
        ...
        getInfos: function (id) {
            ...
            songId = id;
            allmusic();
            songFrom55 = songFroms[responseData.songSheetList[albumId].songTypes[songId]];
            musictype = responseData.songSheetList[albumId].songTypes[songId];
            netmusic();
        },
    ...
    lzxPlayer.playList = {
        creat: {
            album: function (responseData) {
                $('.musicheader', $albumList).html(responseData.siteName + ' - 专辑列表');
                var albumTotal = responseData.songSheetList.length,
                    albumList = '';
                for (var c = 0; c < albumTotal; c++) {
                    albumList += '<li><i class="fa fa-angle-right"></i><span class="index">' + (c + 1)
                        + '</span><span class="artist"></span>《' + responseData.songSheetList[c].songSheetName + "》 - "
                        + responseData.songSheetList[c].author + "</li>";
                }
                ...
                songTotal = responseData.songSheetList[albumId].songIds.length;
                random ? lzxMedia.getInfos(window.parseInt(Math.random() * songTotal))
            ...
            setTimeout(function () {
                ...
                $.ajax({
                   // url: webURL + "/api/musicLyric?songId=" + songSheetList[albumId].songIds[songId] + "&type="
                    ...
                    url: "https://music.cyrilstudio.top/lyric?id=33894312",
                    type: 'GET',
                    dataType: 'script',
                    success: function () {
                        if (lrcstr == '') {
    ...
    let responseData;
    $.ajax({
        url: webURL + '/music/player/songSheet?id=' + keyId,
        type: 'GET',
        dataType: 'json',
        success: function (data) {
            responseData = data
            if(responseData.playerWidth !== -1){
                document.body.style.setProperty('--player-width', playerWidth + 'px');
            }
            if(responseData.coverWidth !== -1){
                document.body.style.setProperty('--cover-width', coverWidth + 'px');
            }
            if(responseData.showNotes !== 1){
                $(".status .note",$player).hide()
            }
            if(responseData.autoPopupPlayer !== -1){
                setTimeout(function () {
                    $player.addClass('show')
                },responseData.autoPopupPlayer * 1000)
            }
            ...
            if ($.cookie("random_play") != null) {
                ...
                if (responseData.randomPlayer !== 1) {
                    $('.loop', $player).addClass(cur);
                    $('.random', $player).removeClass(cur);
                    random = false;
                    $songFrom2.html('<i class="loop fa fa-retweet"></i> 顺序播放');
            ...
            if ($.cookie("lzx_player_volume") == '0.666') {
                volume = (responseData.defaultVolume / 100);
                audio.volume = volume;
            }
            ...
            albumId = responseData.defaultAlbum - 1;

            if (responseData.showLrc == 0) {
                //隐藏歌词
            ...
            if (responseData.showGreeting == 1) {
                lzxTips.show(responseData.greeting);
            }
            setTimeout(function (args) {
                lzxPlayer.playList.creat.album(responseData)
            }, 500)
            console.log("歌曲列表加载完毕!")
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            console.log(XMLHttpRequest.status); // 输出状态码
            console.log(XMLHttpRequest.statusText); // 输出状态文本
            console.log(XMLHttpRequest.responseText); // 输出响应文本
            console.log(XMLHttpRequest.readyState); // 输出当前状态
    ...
    function netmusic() {
        ...
        let playerId = responseData.songSheetList[albumId]
        let cloudMusicApi = "https://music.cyrilstudio.top";
        audio.src = cloudMusicApi + `/song/url?id=${playerId.songIds[songId]}&type=${playerId.songTypes[songId]}`;
        $('.switch-down').show();
        $('.switch-down').html(`<a class="down"><i class="fa fa-cloud-download" title="从${songFrom55}下载：${playerId.songNames[songId]} - ${playerId.artistNames[songId]}"></i></a>`);
        $('.down').click(function () {
            window.open(audio.src, 'newwindow')
        });

        $songName.html(`<span title="${playerId.songNames[songId]}">${LimitStr(playerId.songNames[songId])}</span>`);
        window.console.log(`${name} - 当前播放：${playerId.songNames[songId]} - ${playerId.artistNames[songId]}`);
        $songFrom.html(`<span title="${playerId.artistNames[songId]}">${LimitStr(playerId.artistNames[songId])}</span>`);
        $songFrom1.html(`<span title="${playerId.albumNames[songId]}">${LimitStr(playerId.albumNames[songId])}</span>`);

        var coverImg = new Image();
        coverImg.src = playerId.albumCovers[songId];
        ...
        coverImg.onload = function () {
            ...
            $.ajax({
                url: `${webURL}/music/player/mainColor`,
                type: 'GET',
                dataType: 'json',
                data: {
                    url: coverImg.src
        ...
        coverImg.onerror = function () {
            ...
            coverImg.src = 'https://p2.music.126.net/TjNzzjR2V7gqaLpo9qtpEQ==/18161733068340181.jpg?param=300x300';
            setTimeout(function () {
                lzxTips.show(`${playerId.songNames[songId]} - 专辑图片获取失败！`)
            }, 4000)
        };

        $cover.html(coverImg);

        if (playerId.background === 1) {
            $('.blur-img .blur', $player).attr("src", playerId.albumCovers[songId]); //虚化背景
        } else {
        ...
        if (first == 1) {
            first = 2;
            if (playerId.autoPlayer == 1 && ($.cookie("auto_playre") == null || $.cookie("auto_playre") === "yes")) {
                startPlay()
            } else {
    ...
    function startPlay() {
        lzxTips.show('开始从' + songFrom55 + '播放 - ' + responseData.songSheetList[albumId].songNames[songId]);
        audio.play();
    }
